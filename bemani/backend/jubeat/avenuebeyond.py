...
class JubeatAvenueBeyond(
    ...
    def __get_global_info(self) -> Node:
        ...
        # Lightchat, not really supported but necessary for boot
        lightchat = Node.void("lightchat")
        info.add_child(lightchat)
        map_list = Node.void("map_list")
        lightchat.add_child(map_list)
        map = Node.void("map")
        map_list.add_child(map)
        map.set_attribute("id", "99")
        lcevent_list = Node.void("event_list")
        map.add_child(lcevent_list)
        lcevent = Node.void("event")
        lcevent_list.add_child(lcevent)
        lcevent.set_attribute("id", "1")
        lcevent.add_child(Node.s32("event_type", 1))
        lcevent.add_child(Node.u64("stime", 1672667089))
        lcevent.add_child(Node.u64("etime", 1735796677))
        lcevent.add_child(Node.bool("is_open", True))
        lcevent.add_child(Node.string("hint", "len=64"))
        lcevent.add_child(Node.string("unlock_text", "len=128"))
        lcevent.add_child(Node.void("condition_list"))
        section_list = Node.void("section_list")
        lcevent.add_child(section_list)
    ...
    def format_profile(self, userid: UserID, profile: Profile) -> Node:
        ...
        # JBox stuff
        # May need to replace with jbox_ave
        jbox = Node.void("jbox")
        jboxdict = profile.get_dict("jbox")
        ...
        lightchat.add_child(Node.s32("current_map_id", 99))
        lightchat.add_child(Node.s32("current_event_id", 1))
        map_list = Node.void("map_list")
        lightchat.add_child(map_list)
        map = Node.void("map")
        map_list.add_child(map)
        map.set_attribute("id", "99")
        map.add_child(Node.s32("tune_count", 0))
        map.add_child(Node.u64("last_daily_bonus_time", 1672667089))
        event_list = Node.void("event_list")
        map.add_child(event_list)
        event = Node.void("event")
        event_list.add_child(event)
        event.set_attribute("id", "1")
        event.add_child(Node.s32("display_state", 1))
        event.add_child(Node.void("condition_list"))
        section_list = Node.void("section_list")
        event.add_child(section_list)
        section = Node.void("section")
        section_list.add_child(section)
        section.set_attribute("id", "1")
        section.add_child(Node.s32("acquired_jwatt", 50))
        # section.add_child(Node.s32("acquired_jwatt", profile.get_int("acquired_jwatt")))  # Broken! Fix me!
    ...
    def unformat_profile(self, userid: UserID, request: Node, oldprofile: Profile) -> Profile:
        ...
        # # Lightchat saving and jwatt updating, all commented out because jwatt updating does not work currently
        # lightchat = player.child("lightchat")
        # map_list = lightchat.child("map_list")
        # map = map_list.child("map")
        # lcevent_list = map.child("event_list")
        # lcevent = lcevent_list.child("event")
        # section_list = lcevent.child("section_list")
        ...
        # newprofile.reaplce_int("acquired_jwatt", int(50)) # fixed value

        # Festo dungeon (I still don't know what this is for)
