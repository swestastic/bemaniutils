...
var jubility_view = createReactClass({
    ...
    renderJubilityBreakdown: function(player) {
        if (this.state.version == 15) // beyond the avenue
        return (
            <div className='row'>
                {this.renderAvenueBeyondJubilityTable(player, true)}
                {this.renderAvenueBeyondJubilityTable(player, false)}
            </div>
        );
    ...
    renderAvenueBeyondJubilityTable: function(player, pickup) {
        if (pickup == true)
            jubilityChart = player.pick_up_chart;
        else
            jubilityChart = player.common_chart;
    ...
    renderJubility: function(player) {
        return(
            ...
            :
            // version == avenue beyond
            this.state.version == 15 ? 
            <div>
                <LabelledSection label='Jubility'>
                {(player.common_jubility+player.pick_up_jubility).toFixed(1)}
                </LabelledSection>
