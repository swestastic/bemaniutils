...
class JubeatAvenueClient(BaseClient):
    NAME = "TEST"
    ...
    def verify_shopinfo_ave_regist(self) -> None:
        ...
        shopinfo_ave.set_attribute("method", "regist")
        call.add_child(shopinfo_ave)
        shop = Node.void("shop")
        shopinfo_ave.add_child(shop)
        ...
        self.assert_path(resp, "response/shopinfo_ave/data/cabid")
        self.assert_path(resp, "response/shopinfo_ave/data/locationid")
        self.assert_path(resp, "response/shopinfo_ave/data/tax_phase")
        self.assert_path(resp, "response/shopinfo_ave/data/facility/exist")

        # Verify server flags for events and stuff.
        self.__verify_info(resp, "shopinfo_ave")

    def verify_logger_ave_report(self) -> None:
        call = self.call_node()

        # Construct node
        logger_ave = Node.void("logger_ave")
        call.add_child(logger_ave)
        logger_ave.set_attribute("method", "report")
        logger_ave.add_child(Node.s32("retry", 0))
        data = Node.void("data")
        logger_ave.add_child(data)
        ...
        # Verify that response is correct
        self.assert_path(resp, "response/logger_ave/@status")

    def verify_demodata_ave_get_info(self) -> None:
        call = self.call_node()

        # Construct node
        demodata_ave = Node.void("demodata_ave")
        call.add_child(demodata_ave)
        demodata_ave.set_attribute("method", "get_info")
        pcbinfo = Node.void("pcbinfo")
        demodata_ave.add_child(pcbinfo)
        ...
        # Verify that response is correct
        self.assert_path(resp, "response/demodata_ave/data/info/black_jacket_list")

    def verify_demodata_ave_get_news(self) -> None:
        call = self.call_node()

        # Construct node
        demodata_ave = Node.void("demodata_ave")
        call.add_child(demodata_ave)
        demodata_ave.set_attribute("method", "get_news")

        # Swap with server
        resp = self.exchange("", call)

        # Verify that response is correct
        self.assert_path(resp, "response/demodata_ave/data/officialnews/@count")

    def verify_demodata_ave_get_jbox_list(self) -> None:
        call = self.call_node()

        # Construct node
        demodata_ave = Node.void("demodata_ave")
        call.add_child(demodata_ave)
        demodata_ave.set_attribute("method", "get_jbox_list")

        # Swap with server
        resp = self.exchange("", call)

        # Verify that response is correct
        self.assert_path(resp, "response/demodata_ave/@status")

    def verify_lab_ave_get_ranking(self) -> None:
        call = self.call_node()

        # Construct node
        lab_ave = Node.void("lab_ave")
        call.add_child(lab_ave)
        lab_ave.set_attribute("method", "get_ranking")
        lab_ave.add_child(Node.s32("retry", 0))
        lab_ave.add_child(Node.s8("category", 1))

        # Swap with server
        resp = self.exchange("", call)

        # Verify that response is correct
        self.assert_path(resp, "response/lab_ave/category")
        self.assert_path(resp, "response/lab_ave/entries/@count")

        # Category should be the same as when we requested it.
        if resp.child_value("lab_ave/category") != 1:
    ...
    def __verify_profile(self, resp: Node, should_inherit: bool) -> int:
        for item in [
            ...
            self.assert_path(resp, f"response/gametop_ave/data/player/info/{item}")

        # Technically we could use this to check profile succession doesn't show the
        # tutorial, but we don't go that far yet.
        if resp.child_value("gametop_ave/data/player/info/inherit") != should_inherit:
        ...
        for item in [
            ...
            self.assert_path(resp, f"response/gametop_ave/data/player/item/{item}")
        ...
        for item in [
            ...
            self.assert_path(resp, f"response/gametop_ave/data/player/last/{item}")
        ...
        for item in [
            ...
            self.assert_path(resp, f"response/gametop_ave/data/player/last/settings/{item}")

        # Misc stuff
        self.assert_path(resp, "response/gametop_ave/data/player/session_id")
        self.assert_path(resp, "response/gametop_ave/data/player/event_flag")

        # Profile settings
        self.assert_path(resp, "response/gametop_ave/data/player/name")
        self.assert_path(resp, "response/gametop_ave/data/player/jid")
        if resp.child_value("gametop_ave/data/player/name") != self.NAME:
        ...
        self.assert_path(resp, "response/gametop_ave/data/player/fc_challenge/whim/state")
        self.assert_path(resp, "response/gametop_ave/data/player/official_news/news_list")
        self.assert_path(resp, "response/gametop_ave/data/player/history/@count")
        self.assert_path(resp, "response/gametop_ave/data/player/free_first_play/is_available")
        self.assert_path(resp, "response/gametop_ave/data/player/event_info")
        self.assert_path(resp, "response/gametop_ave/data/player/jbox/point")
        self.assert_path(resp, "response/gametop_ave/data/player/jbox/emblem/normal/index")
        self.assert_path(resp, "response/gametop_ave/data/player/jbox/emblem/premium/index")
        self.assert_path(resp, "response/gametop_ave/data/player/new_music")
        self.assert_path(resp, "response/gametop_ave/data/player/navi/flag")
        self.assert_path(resp, "response/gametop_ave/data/player/gift_list")
        self.assert_path(resp, "response/gametop_ave/data/player/born/status")
        self.assert_path(resp, "response/gametop_ave/data/player/born/year")
        self.assert_path(resp, "response/gametop_ave/data/player/question_list")
        self.assert_path(resp, "response/gametop_ave/data/player/emo_list")
        self.assert_path(resp, "response/gametop_ave/data/player/server")
        self.assert_path(resp, "response/gametop_ave/data/player/course_list")
        self.assert_path(resp, "response/gametop_ave/data/player/course_list/category_list")
        self.assert_path(
            resp,
            "response/gametop_ave/data/player/fill_in_category/normal/no_gray_flag_list",
        )
        self.assert_path(
            resp,
            "response/gametop_ave/data/player/fill_in_category/normal/all_yellow_flag_list",
        )
        self.assert_path(
            resp,
        ...
        self.assert_path(
            resp,
            "response/gametop_ave/data/player/fill_in_category/hard/full_combo_flag_list",
        )
        self.assert_path(
            resp,
            "response/gametop_ave/data/player/fill_in_category/hard/excellent_flag_list",
        )
        self.assert_path(resp, "response/gametop_ave/data/player/department/shop_list")
        self.assert_path(resp, "response/gametop_ave/data/player/stamp/sheet_list")
        self.assert_path(resp, "response/gametop_ave/data/player/festo_dungeon/phase")
        self.assert_path(resp, "response/gametop_ave/data/player/festo_dungeon/clear_flag")

        # Return the jid
        return resp.child_value("gametop_ave/data/player/jid")

    def verify_gameend_ave_regist(
        self,
        ref_id: str,
        jid: int,
        ...
        call.add_child(gameend_ave)
        gameend_ave.set_attribute("method", "regist")
        gameend_ave.add_child(Node.s32("retry", 0))
        pcbinfo = Node.void("pcbinfo")
        gameend_ave.add_child(pcbinfo)
        pcbinfo.set_attribute("client_data_version", "0")
        data = Node.void("data")
        ...
        # Swap with server
        resp = self.exchange("", call)
        self.assert_path(resp, "response/gameend_ave/data/player/session_id")
        self.assert_path(resp, "response/gameend_ave/data/player/end_final_session_id")

    def verify_gameend_ave_final(
        self,
        ref_id: str,
        jid: int,
    ) -> None:
        ...
        gameend_ave = Node.void("gameend_ave")
        call.add_child(gameend_ave)
        gameend_ave.set_attribute("method", "final")
        gameend_ave.add_child(Node.s32("retry", 0))
        pcbinfo = Node.void("pcbinfo")
        gameend_ave.add_child(pcbinfo)
        pcbinfo.set_attribute("client_data_version", "0")
        ...
        # Swap with server
        resp = self.exchange("", call)
        self.assert_path(resp, "response/gameend_ave/@status")

    def verify_gametop_ave_regist(self, card_id: str, ref_id: str) -> int:
        call = self.call_node()

        # Construct node
        gametop_ave = Node.void("gametop_ave")
        call.add_child(gametop_ave)
        gametop_ave.set_attribute("method", "regist")
        data = Node.void("data")
        ...
        # Verify server flags for events and stuff.
        self.__verify_info(resp, "gametop_ave")

        # Verify nodes that cause crashes or failed card-ins if they don't exist
        return self.__verify_profile(resp, False)

    def verify_gametop_ave_get_pdata(self, card_id: str, ref_id: str) -> int:
        call = self.call_node()

        # Construct node
        gametop_ave = Node.void("gametop_ave")
        call.add_child(gametop_ave)
        gametop_ave.set_attribute("method", "get_pdata")
        retry = Node.s32("retry", 0)
        gametop_ave.add_child(retry)
        data = Node.void("data")
        gametop_ave.add_child(data)
        player = Node.void("player")
        data.add_child(player)
    ...
    def verify_gametop_ave_get_mdata(self, jid: int) -> Dict[str, List[Dict[str, Any]]]:
        ret = {}
        for ver in [1, 2, 3]:
            # Construct node
            call = self.call_node()
            gametop_ave = Node.void("gametop_ave")
            call.add_child(gametop_ave)
            gametop_ave.set_attribute("method", "get_mdata")
            retry = Node.s32("retry", 0)
            gametop_ave.add_child(retry)
            data = Node.void("data")
            gametop_ave.add_child(data)
            ...
            # Parse out scores
            self.assert_path(resp, "response/gametop_ave/data/player/jid")
            self.assert_path(resp, "response/gametop_ave/data/player/mdata_list")
            if resp.child_value("gametop_ave/data/player/jid") != jid:
                raise Exception("Unexpected jid received from server!")

            for musicdata in resp.child("gametop_ave/data/player/mdata_list").children:
    ...
    def verify_gametop_ave_get_meeting(self, jid: int) -> None:
        call = self.call_node()

        # Construct node
        gametop_ave = Node.void("gametop_ave")
        call.add_child(gametop_ave)
        gametop_ave.set_attribute("method", "get_meeting")
        gametop_ave.add_child(Node.s32("retry", 0))
        data = Node.void("data")
        gametop_ave.add_child(data)
        player = Node.void("player")
        data.add_child(player)
    ...
    def verify_recommend_ave_get_recommend(self, jid: int) -> None:
        call = self.call_node()

        # Construct node
        recommend_ave = Node.void("recommend_ave")
        call.add_child(recommend_ave)
        recommend_ave.set_attribute("method", "get_recommend")
        recommend_ave.add_child(Node.s32("retry", 0))
        player = Node.void("player")
        recommend_ave.add_child(player)
        player.add_child(Node.s32("jid", jid))
        ...
        # Verify expected nodes
        self.assert_path(resp, "response/recommend_ave/data/player/music_list")

    def verify_demodata_ave_get_hitchart(self) -> None:
        call = self.call_node()

        # Construct node
        gametop_ave = Node.void("demodata_ave")
        call.add_child(gametop_ave)
        gametop_ave.set_attribute("method", "get_hitchart")

        # Swap with server
        resp = self.exchange("", call)

        # Verify expected nodes
        self.assert_path(resp, "response/demodata_ave/data/update")
        self.assert_path(resp, "response/demodata_ave/data/hitchart_lic/@count")
        self.assert_path(resp, "response/demodata_ave/data/hitchart_org/@count")

    def verify_jbox_ave_get_list(self, jid: int) -> None:
        call = self.call_node()

        # Construct node
        jbox_ave = Node.void("jbox_ave")
        call.add_child(jbox_ave)
        jbox_ave.set_attribute("method", "get_list")
        data = Node.void("data")
        jbox_ave.add_child(data)
        player = Node.void("player")
        data.add_child(player)
        player.add_child(Node.s32("jid", jid))
        ...
        # Verify that response is correct
        self.assert_path(resp, "response/jbox_ave/selection_list")

    def verify_jbox_ave_get_agreement(self, jid: int) -> None:
        call = self.call_node()

        # Construct node
        jbox_ave = Node.void("jbox_ave")
        call.add_child(jbox_ave)
        jbox_ave.set_attribute("method", "get_agreement")
        data = Node.void("data")
        jbox_ave.add_child(data)
        player = Node.void("player")
        data.add_child(player)
        ...
        # Verify that response is correct
        self.assert_path(resp, "response/jbox_ave/is_agreement")
    ...
    def verify(self, cardid: Optional[str]) -> None:
        ...
        self.verify_logger_ave_report()
        self.verify_shopinfo_ave_regist()
        self.verify_demodata_ave_get_info()
        self.verify_demodata_ave_get_jbox_list()
        self.verify_demodata_ave_get_news()
        self.verify_demodata_ave_get_hitchart()
        self.verify_lab_ave_get_ranking()
        ...
        if cardid is None:
            ...
            jid = self.verify_gametop_ave_get_pdata(card, ref_id)
            self.verify_recommend_ave_get_recommend(jid)
            scores = self.verify_gametop_ave_get_mdata(jid)
            self.verify_gametop_ave_get_meeting(jid)
            if scores is None:
            ...
            self.verify_jbox_ave_get_list(jid)
            self.verify_jbox_ave_get_agreement(jid)
            self.verify_gameend_ave_final(ref_id, jid)
            ...
            for phase in [1, 2]:
                ...
                self.verify_gameend_ave_regist(ref_id, jid, dummyscores)
                jid = self.verify_gametop_ave_get_pdata(card, ref_id)
                scores = self.verify_gametop_ave_get_mdata(jid)
